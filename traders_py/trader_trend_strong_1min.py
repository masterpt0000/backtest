...
class TradingBot:
    def __init__(self, exchange_name='binance', api_key='', api_secret='', symbol='WLD/USDC:USDC', timeframe='1m', leverage=1, sl_percent=0.03, tp_percent=0.05, input_confirmation_rsi=False, entry_limit_timeout=90, entry_aggressive_after_sec=25, entry_market_fallback=True):
        ...
        # Trailing Stop ATR (Market trend 1min)
        self.use_trailing = False
        self.atr_mult_long = 6.0
        self.atr_mult_short = 6.0
        self._trailing_stop_long = None
        self._trailing_stop_short = None
    ...
    def strategy(self, df):
        ...
        # --- Fast EMA ---
        ema_fast = df['close'].ewm(span=200, min_periods=200, adjust=False).mean()
        df['ema_fast'] = round(ema_fast, 4)
        ema = df['close'].ewm(span=4, min_periods=4, adjust=False).mean()
        df['ema'] = round(ema, 4)
        ...
        df['dif_ema2'] = round(df['ema'] - df['ema'].shift(2), 4)
        df['dif_ema3'] = round(ema_slow - ema_slow.shift(10), 4)

        di_plus, di_minus, adx = adx_indicator(df, 5)
        ...
        # Trailing Stop ATR
        if self.position == 'long' and np.isfinite(atr_value) and np.isfinite(current_close):
            dist = atr_value * self.atr_mult_long
            novo_stop = current_close - dist
            self._trailing_stop_long = max(self._trailing_stop_long or novo_stop, novo_stop)
        elif self.position == 'short' and np.isfinite(atr_value) and np.isfinite(current_close):
            dist = atr_value * self.atr_mult_short
            novo_stop = current_close + dist
            self._trailing_stop_short = novo_stop if self._trailing_stop_short is None else min(self._trailing_stop_short, novo_stop)
        else:
            self._trailing_stop_long = None
            self._trailing_stop_short = None
        ...
        filter_trend = (current_adx > 90)

        print(f"UPPER_6: {current_upper_6}")
        print(f"LOWER_6: {current_lower_6}")
        print(f"ADX: {current_adx} FILTER_TREND: {filter_trend}")
        # ************************************* Strategies first Entry *************************************
        longCondition = filter_trend and current_dif_ema2 > 0.0015
        shortCondition = filter_trend and current_dif_ema2 < -0.0015

        print(f"RSI: {current_rsi} DIF_EMA2: {current_dif_ema2}")
        ...
        signal_result = None
        exit_long = current_rsi < 40
        exit_short = current_rsi > 60

        # Trailing Stop ATR
        hit_trailing_long = self.position == 'long' and self.use_trailing and self._trailing_stop_long is not None and current_close < self._trailing_stop_long
        hit_trailing_short = self.position == 'short' and self.use_trailing and self._trailing_stop_short is not None and current_close > self._trailing_stop_short
        ...
        if hit_trailing_long and self.buyed:
            signal_result = 'sell'
            print(f"Saida: Trailing Stop ATR Long (close={current_close:.4f} < stop={self._trailing_stop_long:.4f})")
            return signal_result
        if hit_trailing_short and self.buyed:
            signal_result = 'sell'
            print(f"Saida: Trailing Stop ATR Short (close={current_close:.4f} > stop={self._trailing_stop_short:.4f})")
            return signal_result
...
if __name__ == "__main__":
    ...
    bot = TradingBot(
        ...
        sl_percent=0.03,  # 3% stop-loss (Pine)
        tp_percent=2.00   # 5% take-profit (Pine)
    )
    bot.run()