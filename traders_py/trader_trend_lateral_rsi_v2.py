...
class TradingBot:
    def __init__(self, exchange_name='binance', api_key='', api_secret='', symbol='WLD/USDC:USDC', timeframe='1m', leverage=1, sl_percent=0.03, tp_percent=0.007, input_confirmation_rsi=False, entry_limit_timeout=90, entry_aggressive_after_sec=25, entry_market_fallback=True):
        ...
        # Cooldown após saída (evita re-entrada imediata, como count no Pine)
        self.entry_cooldown_count = 0
    ...
    def get_total_balance(self):
        ...
        # Deixar ~5% de margem de segurança para fees e arredondamentos
        self.quantity = balance_to_use * 0.95
    ...
    def strategy(self, df):
        ...
        atr = ma_function(tr_series, 100, "WMA")
        df['atr'] = round(atr, 4)

        # --- Fast EMA ---
        ema_fast = df['close'].ewm(span=100, min_periods=100, adjust=False).mean()
        ...
        current_dif_adx = df['dif_ADX'].iloc[last_idx]
        ...
        ema2_is_flat = abs(float(current_dif_ema2)) <= 0.001
        if self.position is not None:
            self.flat_ema2_count = self.flat_ema2_count + 1 if ema2_is_flat else 0
        else:
            self.flat_ema2_count = 0
        
        filter_trend = (atr_value < 0.00399 and current_lower_6 < current_close and current_upper_6 > current_close and current_dif_ema <= 0.0005999 and current_dif_ema >= -0.0005999 and current_dif_adx < 5 and current_dif_obv < 400000 and current_dif_obv > -400000)

        if self.entry_cooldown_count > 0:
            self.entry_cooldown_count = self.entry_cooldown_count - 1

        print(f"ATR: {atr_value}")
        print(f"DIF_EMA: {current_dif_ema}")
        print(f"ADX: {current_adx}")
        print(f"DIF_ADX: {current_dif_adx}")
        print(f"FILTER_TREND: {filter_trend}")
        # ************************************* Strategies first Entry *************************************
        longCondition = filter_trend and current_rsi < 35 and current_rsi_vwap < 20
        shortCondition = filter_trend and current_rsi > 65 and current_rsi_vwap > 80

        print(f"LOWER_6: {current_lower_6}")
        print(f"RSI: {current_rsi}")
        print(f"OBV: {current_obv}, DIF_OBV: {current_dif_obv}")
        ...
        signal_result = None
        exit_rsi_vwap_long = (current_dif_ema2 < 0 and current_rsi > 47)
        exit_rsi_vwap_short = (current_dif_ema2 > 0 and current_rsi < 53)
        Strategy_exit_long = (current_rsi > 50) or exit_rsi_vwap_long
        Strategy_exit_short = (current_rsi < 50) or exit_rsi_vwap_short

        # ************************************* Do Entry *************************************
        if (longCondition and (self.position is None or self.position == 'short') and self.entry_cooldown_count == 0):
            signal_result = 'long'
            return signal_result

        if (shortCondition and (self.position is None or self.position == 'long') and self.entry_cooldown_count == 0):
            signal_result = 'short'
            return signal_result

        # ************************************* Do Exit *************************************
        # Saída por lateralização: EMA2 flat 15 candles consecutivos
        if self.position is not None and self.flat_ema2_count >= 15 and self.buyed:
            signal_result = 'sell'
            self.entry_cooldown_count = 10
            print(f"[SAIDA] EMA2 flat 15 candles (flat_ema2_count={self.flat_ema2_count})")
            return signal_result

        if self.position == 'long' and Strategy_exit_long and self.buyed:
            signal_result = 'sell'
            if exit_rsi_vwap_long:
                self.entry_cooldown_count = 10
            return signal_result

        if self.position == 'short' and Strategy_exit_short and self.buyed:
            signal_result = 'sell'
            if exit_rsi_vwap_short:
                self.entry_cooldown_count = 10
            return signal_result
    ...
    def _place_limit_close_and_wait(self, side, quantity_str, num_attempts=6, timeout_per_attempt=20):
