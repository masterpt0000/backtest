...
def _ensure_ohlcv_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Nova_estrategia_v2: faltam colunas OHLCV: {missing} (tem {list(df.columns)})")
...
def strategy(self, df):
    ...
    long_signal = bool(((_fv(df["t1"], cur_i)) < (20))) and market_ok
    short_signal = bool(((_fv(df["t1"], cur_i)) > (80))) and market_ok
...
if __name__ == "__main__":
    bot = TradingBot(
        ...
        buyed=False,
        strategy_name="Nova_estrategia_v2",
        type_strategy="trend",
